<script lang="ts">
	...
	let { images, startMaximized, done } = $props();
	import Image from './Image.svelte';
	import { cls } from 'svelte-ux';
	import { boxTransition } from '$lib/utils/transitions';
	import { switchCase } from '$lib/utils/switch-case';
	...
	function onImageClicked(event: PointerEvent, index: number) {
...
{#if selectedIndex !== null && previewDimensions}
	<div
		in:boxTransition="{{
			...
			end: { x: 0, y: 0, width: windowWidth, height: windowHeight }
		}}"
		out:boxTransition="{{
			delay: 0,
		...
		style={
			switchCase({
				true: 'left: 0; top: 0; width: 100vw; height: 100vh;',
				false: `left: ${previewDimensions.x}px; top: ${previewDimensions.y}px; width: ${previewDimensions.width}px; height: ${previewDimensions.height}px;`
			})(selectedIndex !== null)
		}
